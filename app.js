...
function displayProducts(productsToShow) {
  ...
  productsToShow.forEach((product) => {
    ...
    card.className =
      "bg-white/70 backdrop-blur-md rounded-2xl shadow-md hover:shadow-2xl p-5 transition-all duration-300 hover:scale-105 hover:border-indigo-200 border border-slate-200/50 group"
    card.innerHTML =
      ...
      "<h3 class='font-bold text-sm line-clamp-2 mb-3 text-slate-900'>" +
      product.title +
      "</h3>" +
      "<p class='bg-gradient-to-r from-indigo-600 to-rose-500 bg-clip-text text-transparent font-bold text-lg mb-2'>" +
      product.price +
      " BDT</p>" +
      "<p class='text-amber-500 text-sm mb-4 font-semibold'>⭐ " +
      product.rating.rate +
      "</p>" +
      "<button class='w-full bg-gradient-to-r from-indigo-600 to-rose-500 text-white py-2 rounded-lg hover:shadow-lg font-bold add-to-cart-btn transition-all duration-200 active:scale-95 hover:scale-105' data-id='" +
      product.id +
      "'>Add to Cart</button>"
...
function updateCart() {
  ...
  if (cart.length === 0) {
    cartItems.innerHTML = "<p class='text-slate-500 text-center font-medium'>Cart is empty</p>"
    updateSummary()
    return
  }
  ...
  cart.forEach((item) => {
    ...
    div.className =
      "bg-gradient-to-r from-slate-50 to-indigo-50 p-4 rounded-xl border border-slate-200 hover:shadow-md transition-all duration-300"
    div.innerHTML =
      "<div class='flex justify-between mb-3'><div><p class='font-bold line-clamp-1 text-slate-900'>" +
      item.title +
      "</p><p class='text-slate-600 font-semibold'>" +
      item.price +
      " BDT</p></div>" +
      "<button class='bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded-lg remove-btn transition-all duration-200 hover:scale-110 active:scale-95' data-id='" +
      item.id +
      "'>✕</button></div>" +
      "<div class='flex justify-between items-center border border-indigo-300 rounded-lg bg-white'><button class='bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 decrease-btn transition-colors' data-id='" +
      item.id +
      "'>−</button>" +
      "<span class='px-4 font-bold text-slate-900'>Qty: " +
      item.quantity +
      "</span>" +
      "<button class='bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 increase-btn transition-colors' data-id='" +
      item.id +
      "'>+</button></div>"
